...
| 1 | `#484f74` | 0.437 / 0.061 | |
| 2 | `#515ba8` | 0.502 / 0.122 | |
| 3 (max) | `#5962eb` | 0.567 / 0.204 | ~1.3x `primary`'s own chroma (0.159) at the same lightness and hue |

Only 3 nonzero levels (not the usual 4-5): the achievable lightness range between `surface-1` and this ramp's own top lightness is narrow, and 4 steps couldn't clear the ≥0.06 adjacent-ΔL gate at this contrast floor — validated empirically, not assumed. Lightness is unchanged from the 3rd pass (adjacent ΔL ~0.065 per step, level 1 as light as the gap budget allows with level 3 fixed). Chroma is a fixed 30%/60%/100% fraction of level 3's own chroma at levels 1/2/3 (down from 40%/72%/100% in the 3rd pass — user's explicit target on this 4th pass) — still strictly increasing by construction, so level 2 can't overshoot level 3 the way it did before that got fixed. Re-derive (same method) if the locked palette above ever changes.
...
|---|---|---|---|
| 1 | `#484f74` | `#7a584c` | `#4f675b` |
| 2 | `#515ba8` | `#a45e45` | `#4c8068` |
| 3 (max) | `#5962eb` | `#d65c2d` | `#349c73` |

All three ramps independently pass the same `--ordinal` checks as the primary one (monotone lightness, ≥0.06 adjacent ΔL, ≥2:1 dark-end contrast, single hue) — level 1's contrast against `surface-1` is now 2.40:1 / 3.02:1 / 3.11:1 (up slightly from the 3rd pass, since less chroma at the same lightness reads as marginally higher WCAG luminance contrast). The level-3 triple `#5962eb` / `#d65c2d` / `#349c73` (unchanged since the saturation-boost pass, re-validated then) is what the per-source legend and single-source-day cells actually paint — still passes its own categorical CVD check (worst pair ΔE 9.3).
...
- `2026-09-01` (fourth pass) — User gave explicit target fractions: chroma at 30%/60%/100% of level 3's own chroma (down from the 3rd pass's 40%/72%/100%). Lightness values untouched from the 3rd pass (level 3 still fixed, unchanged since the saturation-boost pass). Re-derived levels 1-2's chroma directly from these fractions and re-ran `validate_palette.js --ordinal` per ramp — all PASS, and level 1's contrast margin actually improved slightly (2.40:1/3.02:1/3.11:1) since less chroma at a fixed lightness reads as marginally higher WCAG contrast.
