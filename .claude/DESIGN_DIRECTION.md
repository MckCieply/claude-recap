...
| 1 | `#363a9f` | 0.410 / 0.161 | |
| 2 | `#453ee0` | 0.488 / 0.235 | |
| 3 (max) | `#5962eb` | 0.567 / 0.204 | ~1.3x `primary`'s own chroma (0.159) at the same lightness and hue |

Only 3 nonzero levels (not the usual 4-5): the achievable lightness range between `surface-1` and this ramp's own top lightness is narrow, and 4 steps couldn't clear the ≥0.06 adjacent-ΔL gate at this contrast floor — validated empirically, not assumed. Adjacent ΔL is ~0.079 per step (levels 1-2 got a moderate lightness-gap widening in an earlier pass, then this pass kept the same lightnesses and boosted chroma instead — see decision log). Chroma is now ~1.85x the pre-boost values at every level (35%/72%/100% of level 3's chroma before → same relative shape now, just against a taller level-3 chroma), capped at 85% of the sRGB gamut ceiling for each level's own lightness/hue so nothing clips or looks artificial at the edge. Re-derive (same method) if the locked palette above ever changes.
...
|---|---|---|---|
| 1 | `#363a9f` | `#7e2d0b` | `#025237` |
| 2 | `#453ee0` | `#a74620` | `#257555` |
| 3 (max) | `#5962eb` | `#d65c2d` | `#349c73` |

All three ramps independently pass the same `--ordinal` checks as the primary one (monotone lightness, ≥0.06 adjacent ΔL, ≥2:1 dark-end contrast, single hue) — orange and aqua land wider lightness gaps than primary (~0.106 and ~0.117 ΔL per step, vs. ~0.079 for primary) simply because their level-3 anchors are lighter (`L` 0.622 / 0.621 vs. primary's 0.567), leaving more room before the level-1 contrast floor. The level-3 triple `#5962eb` / `#d65c2d` / `#349c73` is re-validated as its own categorical set (not assumed from the original brand-color set's PASS, since chroma shifted): worst all-pairs CVD ΔE 9.3 (deutan/tritan, still above the 8.0 target), worst normal-vision ΔE 24.7, all ≥3:1 contrast — all PASS.
