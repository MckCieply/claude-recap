...
**Multi-file import:** per-source color (for showing multiple imported export files at once) is a deliberate, documented exception to the single-accent rule — validated categorical palette, per-source ramps, and the heatmap corner-gradient mechanic are all specified in [.claude/DESIGN_DIRECTION.md](.claude/DESIGN_DIRECTION.md) under "Data visualization — categorical palette." Read that before touching multi-source rendering; don't invent a blend/legend approach ad hoc.
